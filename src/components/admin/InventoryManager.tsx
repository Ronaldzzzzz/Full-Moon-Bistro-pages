      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-2">
          {items.map((item) => (
            <div
              key={item.id}
              className="flex items-center gap-3 bg-[#2a2015] border border-[#4a3820] rounded p-3"
            >
              <div className="flex-1">
                <span className="text-[#d4c090] text-sm font-semibold">{item.name}</span>
                {item.note && (
                  <span className="text-[#6a5030] text-xs ml-2">{item.note}</span>
                )}
              </div>
              <span className="text-[#c9a55a] text-sm font-semibold">
